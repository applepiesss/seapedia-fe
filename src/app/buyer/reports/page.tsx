...
import { getBuyerReport } from "@/lib/buyerApi";
...
export default function BuyerReportsPage() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        ...
        if (!token) return;
        const data = await getBuyerReport(token);
        setReport(data);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto p-8">
      <h1 className="text-3xl font-bold mb-8 text-gray-800">Spending Report</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center">
          <p className="text-gray-500 mb-2 font-medium">Total Spent</p>
          <p className="text-3xl font-bold text-gray-800">Rp {report.totalSpent.toLocaleString("id-ID")}</p>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center">
          <p className="text-gray-500 mb-2 font-medium">Total Orders</p>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="p-4 font-semibold text-gray-600">Final Total</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {report.orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="p-4 text-gray-800">{order.storeName}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center">
                    <Link href={`/buyer/orders/${order.id}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm">
                      View
                    </Link>
                  </td>
                </tr>
